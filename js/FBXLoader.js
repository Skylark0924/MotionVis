...
// 兼容性处理：ColorManagement可能不存在
let ColorManagement;
if (THREE.ColorManagement && THREE.ColorManagement.colorSpaceToWorking) {
  ColorManagement = THREE.ColorManagement;
} else {
  // 创建简单的ColorManagement替代
  ColorManagement = {
    colorSpaceToWorking: function(color, colorSpace) {
      // 简单返回原色，不做颜色空间转换
      return color;
    }
  };
...
// 创建安全的颜色空间转换函数
function safeColorSpaceToWorking(color, colorSpace) {
  try {
    if (ColorManagement && ColorManagement.colorSpaceToWorking) {
      return ColorManagement.colorSpaceToWorking(color, colorSpace);
    } else {
      // 如果ColorManagement不可用，直接返回原色
      return color;
    }
  } catch (error) {
    console.warn('FBXLoader: ColorManagement.colorSpaceToWorking failed, using original color:', error);
    return color;
  }
}

// 移除import语句，使用全局fflate对象（如果存在）
// 如果没有fflate，我们需要提供一个替代方案
let fflate;

// 使用SimpleZlib替代fflate
console.log('FBXLoader: 检查压缩库...');

if (typeof window !== 'undefined' && window.simpleZlib) {
  fflate = window.simpleZlib;
  console.log('FBXLoader: SimpleZlib library loaded successfully');
  console.log('SimpleZlib version:', fflate.version);
} else if (typeof window !== 'undefined' && window.fflate) {
  fflate = window.fflate;
  console.log('FBXLoader: fflate library loaded successfully');
} else {
  console.warn('FBXLoader: no compression library available, creating fallback');
  // 创建替代方案
  fflate = {
    unzlibSync: function(data) {
      console.warn('FBXLoader: no compression library available, compressed data may not load correctly.');
      // 尝试检测是否为zlib压缩数据
      if (data.length >= 2) {
        const header = data[0] << 8 | data[1];
        // zlib header: 0x78 0x9C, 0x78 0xDA, 0x78 0x5E, 0x78 0x01
        if ((header & 0xFF00) === 0x7800) {
          console.error('FBXLoader: Detected zlib compressed data but no compression library is available.');
          console.error('FBXLoader: Please use uncompressed FBX files or include a compression library.');
          // 返回一个空的数据缓冲区，避免边界错误
...
// 移除NURBSCurve的import，如果THREE中没有，我们创建一个简单的替代
let NURBSCurve;
if (THREE.NURBSCurve) {
  NURBSCurve = THREE.NURBSCurve;
} else {
  // 创建一个简单的NURBSCurve替代
  NURBSCurve = class {
    ...
    getPoints(divisions) {
      ...
      for (let i = 0; i < divisions; i++) {
        ...
        if (this.controlPoints.length > 0) {
          const index = Math.floor(t * (this.controlPoints.length - 1));
          point.copy(this.controlPoints[index]);
        }
        points.push(point);
      }
      return points;
...
class FBXTreeParser {
  ...
  loadTexture(textureNode, images) {
    ...
    if (fileName === undefined) {
      console.warn(
        "FBXLoader: Undefined filename, creating placeholder texture."
      );
      return new Texture();
    }
  ...
  parseParameters(materialNode, textureMap, ID) {
    ...
    if (materialNode.Diffuse) {
      parameters.color = safeColorSpaceToWorking(
    ...
    if (materialNode.Emissive) {
      parameters.emissive = safeColorSpaceToWorking(
    ...
    if (materialNode.Specular) {
      parameters.specular = safeColorSpaceToWorking(
...
class GeometryParser {
  ...
  parseVertexColors(ColorNode) {
    ...
    for (let i = 0, c = new Color(); i < buffer.length; i += 4) {
      ...
      safeColorSpaceToWorking(c, SRGBColorSpace);
      c.toArray(buffer, i);
    }
...
class BinaryReader {
  ...
  // 添加边界检查方法
  checkBounds(bytes) {
    if (this.offset + bytes > this.dv.byteLength) {
      // 如果缓冲区为空或太小，返回默认值而不是抛出错误
      if (this.dv.byteLength === 0) {
        console.warn('THREE.FBXLoader: Empty buffer detected, returning default values');
        return false; // 表示不需要读取
      }
      throw new Error(`THREE.FBXLoader: Attempting to read beyond buffer bounds. Offset: ${this.offset}, Requested bytes: ${bytes}, Buffer size: ${this.dv.byteLength}`);
    }
    return true; // 表示可以安全读取
  }
  ...
  // then sees LSB.
  getBoolean() {
    if (!this.checkBounds(1)) return false;
  ...
  getInt32Array(size) {
    if (!this.checkBounds(size * 4)) return new Array(size).fill(0);
    const a = [];
  ...
  getInt64Array(size) {
    if (!this.checkBounds(size * 8)) return new Array(size).fill(0);
    const a = [];
  ...
  getArrayBuffer(size) {
    if (!this.checkBounds(size)) return new ArrayBuffer(0);
  ...
  getString(size) {
    if (!this.checkBounds(size)) return '';
